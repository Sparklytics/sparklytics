...
import type { Goal, GoalType, MatchOperator } from '@/lib/api';
...
export function GoalFormDialog({ websiteId, open, onClose, editingGoal }: GoalFormDialogProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (isEditing && editingGoal) {
      updateGoal.mutate(
        { goalId: editingGoal.id, payload: { name: name.trim(), match_value: matchValue.trim(), match_operator: matchOperator } },
        {
          onSuccess: () => onClose(),
      ...
      createGoal.mutate(
        { name: name.trim(), goal_type: goalType, match_value: matchValue.trim(), match_operator: matchOperator },
        {
          onSuccess: () => onClose(),
          onError: (err) => setApiError(err.message),
        }
